/***************************************************************************
    ...
    Project              : QtiPlot
    --------------------------------------------------------------------
	Copyright            : (C) 2006 - 2011 by Ion Vasilief, 2006 by Knut Franke
...
void LineProfileTool::calculateLineProfile(const QPoint& start, const QPoint& end)
{
	...
	QImage image = d_target->pixmap().toImage();
...
double LineProfileTool::averageImagePixel(const QImage& image, int px, int py, bool moreHorizontal)
{
	...
	if (moreHorizontal){
		start = py - middle;
		for (int i = 0; i < d_average_pixels; i++){
			QRgb pixel = image.pixel(px, start + i);
			sum += qGray(pixel);
		}
	} else {
		start = px - middle;
		for (int i = 0; i < d_average_pixels; i++){
			QRgb pixel = image.pixel(start + i, py);
			sum += qGray(pixel);
		}
	}
