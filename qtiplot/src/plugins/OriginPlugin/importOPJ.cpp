		...
		for(int j = 0; j < columnCount; ++j){
			...
			for(unsigned int i = 0; i < column.data.size(); ++i){
				...
				if(column.valueType != Origin::Text){// number
					if(value.type() == Origin::Variant::V_STRING){//Origin::TextNumeric column should be set to Text
						has_texts = true;
						table->setText(i, j, QString(value.as_string()));
					}
					...
					table->setText(i, j, QString(value.type() == Origin::Variant::V_STRING ? value.as_string() : ""));
				}
			}
	...
	for(unsigned int g = 0; g < opj.graphCount(); ++g){
		...
		for(unsigned int l = 0; l < layers; ++l){
			...
			for(unsigned int c = 0; c < layer.curves.size(); ++c){
				...
				switch(_curve.symbolShape&0xFF){
					case 0: //NoSymbol
						cl.sType=0;
						break;
					case 1: //Rect
