/***************************************************************************
	...
	Copyright            : (C) 2007-2008 Alex Kargovsky
						   (C) 2010-2011 Ion Vasilief
...
void Origin750Parser::readColumnValues(unsigned int spread, unsigned int col, short data_type, char valuesize, unsigned int nr, bool validColumn)
{
	if (spread >= speadSheets.size())
		return;
	...
	for (unsigned int i = 0; i < nr; ++i){
		...
		if(valuesize <= 8)	// Numeric, Time, Date, Month, Day
		{
			...
			if (validColumn)
				speadSheets[spread].columns[col].data.push_back(value);
		}
		else if((data_type & 0x100) == 0x100) // Text&Numeric
		{
			...
			else //text
			{
				...
				if(stmp.find(0x0E) != string::npos) // try find non-printable symbol - garbage test
					stmp = string();
				if (validColumn)
					speadSheets[spread].columns[col].data.push_back(stmp);
...
void Origin750Parser::readMatrixValues(short data_type, char data_type_u, char valuesize, unsigned int size, int mIndex)
{
	if (matrices.empty())
		return;

	if (mIndex < 0)
		mIndex = matrices.size() - 1;
	...
	switch(data_type){
		case 0x6001://double
			for(unsigned int i = 0; i < size; ++i){
				double value;
				file >> value;
				matrices[mIndex].sheets.back().data.push_back((double)value);
			}
			break;
		case 0x6003://float
			for(unsigned int i = 0; i < size; ++i){
	...
	if (logValues){
		...
		for(unsigned int i = 0; i < 10 && i < matrices[mIndex].sheets.back().data.size(); ++i)
			LOG_PRINT(logfile, "%g\t", matrices[mIndex].sheets.back().data[i]);
	}
}
...
void Origin750Parser::readFunction(unsigned int colpos, char valuesize, unsigned int *oldpos)
{
	Origin::Function &f = functions.back();
	file >> f.formula.assign(valuesize, 0);
	f.formula = toLowerCase(f.formula);
	*oldpos = file.tellg();
	...
	LOG_PRINT(logfile, "NEW FUNCTION: %s = %s\n", f.name.c_str(), f.formula.c_str());
...
void Origin750Parser::readSpreadInfo()
{
	...
	unsigned int spread = findSpreadByName(name);
	if (spread < 0 || spread >= speadSheets.size())
		return;
	...
	while(!file.eof()){
		...
		if(col_index != -1){
			if (speadSheets[spread].columns[col_index].name != name)
				speadSheets[spread].columns[col_index].name = name;
...
void Origin750Parser::readExcelInfo()
{
	...
	LAYER += size + 0x1;
	int isheet = 0;
	while(!file.eof()){// multisheet loop
		// LAYER section
		...
		while(!file.eof()){
			...
			if(col_index != -1){
				...
				LOG_PRINT(logfile, "				COLUMN %s has type = %d(%d) (@ 0x%X)\n", excels[iexcel].sheets[isheet].columns[col_index].name.c_str(), type, (int)c, (LAYER + 0x11));
			}
			LAYER += 0x1E7 + 0x1;
...
void Origin750Parser::readMatrixInfo()
{
	...
	MatrixSheet sheet = matrices[idx].sheets.back();
	...
	matrices[idx].sheets.back() = sheet;
...
void Origin750Parser::readProjectTree()
{
	if (file.eof())
		return;
	tree<ProjectNode>::iterator top = projectTree.insert(projectTree.begin(), ProjectNode());
	readProjectTreeFolder(top);
...
void Origin750Parser::readGraphAxisPrefixSuffixInfo(const string& sec_name, unsigned int size, GraphLayer& layer)
{
	if(sec_name == "PL"){
		...
		string text(size, 0);
		file >> text;
		layer.yAxis.formatAxis[0].factor = text;
	} else if(sec_name == "OR"){
		string text(size, 0);
		file >> text;
		layer.yAxis.formatAxis[1].factor = text;
	} else if(sec_name == "OB"){
		string text(size, 0);
		file >> text;
		layer.xAxis.formatAxis[0].factor = text;
	} else if(sec_name == "OT"){
		string text(size, 0);
		file >> text;
		layer.xAxis.formatAxis[1].factor = text;
	}
}
