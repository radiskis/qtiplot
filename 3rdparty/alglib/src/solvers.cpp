     ...
     ae_state *_state)
{
    ...
    if( !sparseiscrs(a, _state) )
    {
        ...
        if( isupper )
        {
            ...
            flg = spsymmanalyze(&a3, &idummy, 0.0, 0, facttype, 0, donotreusemem, &analysis, _state);
        }
        else
        {
            flg = spsymmanalyze(&a2, &idummy, 0.0, 0, facttype, 0, donotreusemem, &analysis, _state);
        }
    }
    else
    {
        ...
        {
            flg = spsymmanalyze(a, &idummy, 0.0, 0, facttype, 0, donotreusemem, &analysis, _state);
        }
    }
    if( !flg )
    {
     ...
     ae_state *_state)
{
    ...
    if( !spsymmanalyze(&aug, &priorities, 0.0, 0, facttype, priorityordering, donotreusememory, &analysis, _state) )
    {
