...
/* state flags */
#define _ALGLIB_FLG_THREADING_MASK_WRK              0x7ULL
#define _ALGLIB_FLG_THREADING_MASK_CBK              (0x7ULL<<3)
#define _ALGLIB_FLG_BACKEND_MASK_LINALG             (0x3ULL<<6)
#define _ALGLIB_FLG_BACKEND_MASK_DSS                (0x3ULL<<8)
#define _ALGLIB_FLG_USE_GLOBAL                      0x0ULL

#define _ALGLIB_FLG_THREADING_MASK_ALL              (_ALGLIB_FLG_THREADING_MASK_WRK|_ALGLIB_FLG_THREADING_MASK_CBK)
#define _ALGLIB_FLG_THREADING_SERIAL                0x1ULL
#define _ALGLIB_FLG_THREADING_PARALLEL              0x2ULL
#define _ALGLIB_FLG_THREADING_SERIAL_CALLBACKS      (0x1ULL<<3)
#define _ALGLIB_FLG_THREADING_PARALLEL_CALLBACKS    (0x2ULL<<3)

#define _ALGLIB_FLG_BACKEND_LINALG                  (0x1ULL<<6)
#define _ALGLIB_FLG_BACKEND_NOLINALG                (0x2ULL<<6)

#define _ALGLIB_FLG_BACKEND_DSS                     (0x1ULL<<8)
#define _ALGLIB_FLG_BACKEND_NODSS                   (0x2ULL<<8)

#define _ALGLIB_FLG_BACKEND_USE_ALL                 (_ALGLIB_FLG_BACKEND_LINALG|_ALGLIB_FLG_BACKEND_DSS)

#define _ALGLIB_FLG_WORKER_THREAD                   (0x1ULL<<63)
...
#error ALGLIB was requested to fail without FMA intrinsics
        #endif
    #endif
#elif defined(AE_CPU) && (AE_CPU==AE_AARCH64)
    /*
     * AArch64 (ARM V8+) definitions
     */
#endif
...
typedef void(*ae_opaque_copy_constructor)(void*,const void*,ae_state*);
...
typedef struct ae_smart_ptr
{
    ...
    /* whether ownership can be released or not;
       if exclusive, any attempt to transfer ownership will result in an exception;
       the pointer can be free, though
       */
    ae_bool exclusive;
...
typedef struct ae_opaque_object
{   
    /* opaque object pointer; NULL for uninitialized */
    void *ptr;
    
    /* opaque object type, an integer constant, used to perform integrity checks by consumers; zero for uninitialized */
    ae_int_t obj_type;
    
    /* opaque object size, >0 for initialized, zero for uninitialized */
    ae_int_t obj_size;
    
    /* copy constructor for the pointer */
    ae_opaque_copy_constructor opaque_copy_constructor;
    
    /* destructor function for pointer; clears all dynamically allocated memory */
    ae_destructor destructor;
    
    /* frame entry; used to ensure automatic deallocation of object in case of exception/exit */
    ae_dyn_block frame_entry;
} ae_opaque_object;

static const ae_int_t _ALGLIB_PBL_DSS = 1;
...
************************************************************************/
void ae_set_global_flags(ae_uint64_t flg_value);
ae_uint64_t ae_get_global_flags();
#if defined(_ALGLIB_HAS_THREADLOCAL)
ae_int_t ae_get_callback_worker_idx();
void ae_set_callback_worker_idx(ae_int_t idx);
#endif
ae_int_t get_nprocessors_cached();

static void ae_set_global_threading(ae_uint64_t flg_value) { ae_set_global_flags(flg_value); }
...
void ae_really_weak_store_u64(ae_uint64_t *p, ae_uint64_t v);
...
ae_uint64_t ae_get_effective_flags(ae_state *state);
...
void ae_smart_ptr_init(ae_smart_ptr *dst, void **subscriber, ae_bool exclusive, ae_state *state, ae_bool make_automatic);
...
void ae_opaque_object_init(ae_opaque_object *dst, ae_state *state, ae_bool make_automatic);
void ae_opaque_object_init_copy(ae_opaque_object *dst, const ae_opaque_object *src, ae_state *state, ae_bool make_automatic);
void ae_opaque_object_clear(void *_dst); /* accepts ae_opaque_object* */
void ae_opaque_object_destroy(void *_dst);
...
void ae_obj_array_extract_transfer(ae_obj_array *arr, ae_int_t idx, ae_smart_ptr *ptr, ae_state *state);
void ae_obj_array_pop_transfer(ae_obj_array *arr, ae_smart_ptr *ptr, ae_state *state);
void ae_obj_array_swap(ae_obj_array *arr, ae_int_t idx, ae_int_t idx2, ae_state *state);
...
void ae_shared_pool_set_seed(
    ...
    ae_destructor   destructor,
    ae_state        *state);
void ae_shared_pool_set_seed_if_different(
    ae_shared_pool  *dst,
    const void      *seed_object,
    ae_int_t        size_of_object,
    ae_copy_constructor copy_constructor,
...
************************************************************************/
    ...
    /*
     * Updates CPU dispatcher entry with a pointer to the appropriate function;
     * does not use atomics when it is safe to do (aligned stores on x64).
     *
     * Crashes when the very basic assumptions about alignment are violated.
     */
    #if AE_CPU==AE_INTEL && !defined(_ALGLIB_TSAN)
        #define _ALGLIB_ATOMIC_SAVE_FPTR(ptr_name, func_name) \
        { \
            if( (sizeof(ptr_name)!=sizeof(void*)) || (ae_misalignment(&ptr_name,sizeof(void*))!=0) ) \
                { fprintf(stderr, "ALGLIB: critical error during _ALGLIB_ATOMIC_SAVE_FPTR() for %s()\n", #ptr_name); fflush(stderr); abort(); }  \
            ptr_name = func_name; \
        }
    #else
        #define _ALGLIB_ATOMIC_SAVE_FPTR(ptr_name, func_name)
     ...
     */
        ...
        #define _ALGLIB_KSV_VOID_AVX2(fname,params)   if( cached_cpuid&CPU_AVX2 ) { _ALGLIB_ATOMIC_SAVE_FPTR(fname, fname##_avx2); fname##_avx2 params; return; }
        #define _ALGLIB_KSV_RETURN_AVX2(fname,params) if( cached_cpuid&CPU_AVX2 ) { _ALGLIB_ATOMIC_SAVE_FPTR(fname, fname##_avx2); return fname##_avx2 params; }
    #else
        #define _ALGLIB_KKK_VOID_AVX2(fname,params)
        #define _ALGLIB_KKK_RETURN_AVX2(fname,params)
        #define _ALGLIB_KSV_VOID_AVX2(fname,params)
        #define _ALGLIB_KSV_RETURN_AVX2(fname,params)
    #endif
        ...
        #define _ALGLIB_KSV_VOID_FMA(fname,params)    if( cached_cpuid&CPU_FMA )  { _ALGLIB_ATOMIC_SAVE_FPTR(fname, fname##_fma); fname##_fma params; return; }
        #define _ALGLIB_KSV_RETURN_FMA(fname,params)  if( cached_cpuid&CPU_FMA )  { _ALGLIB_ATOMIC_SAVE_FPTR(fname, fname##_fma); return fname##_fma params; }
    #else
        #define _ALGLIB_KKK_VOID_FMA(fname,params)
        #define _ALGLIB_KKK_RETURN_FMA(fname,params)
        #define _ALGLIB_KSV_VOID_FMA(fname,params)
        #define _ALGLIB_KSV_RETURN_FMA(fname,params)
    #endif
        ...
        #define _ALGLIB_KERNELSAVE_VOID_AVX2_FMA(fname,params) \
        {\
            ae_int_t cached_cpuid = ae_cpuid();\
            _ALGLIB_KSV_VOID_FMA(fname,params)\
            _ALGLIB_KSV_VOID_AVX2(fname,params)\
        }
        #define _ALGLIB_KERNELSAVE_RETURN_AVX2_FMA(fname,params) \
        {\
            ae_int_t cached_cpuid = ae_cpuid();\
            _ALGLIB_KSV_RETURN_FMA(fname,params)\
            _ALGLIB_KSV_RETURN_AVX2(fname,params)\
        }
    #else
        #define _ALGLIB_KERNEL_VOID_AVX2_FMA(fname,params) {}
        #define _ALGLIB_KERNEL_RETURN_AVX2_FMA(fname,params) {}
        ...
        #define _ALGLIB_KERNELSAVE_VOID_FMA(fname,params) \
        {\
            ae_int_t cached_cpuid = ae_cpuid();\
            _ALGLIB_KSV_VOID_FMA(fname,params)\
        }
        #define _ALGLIB_KERNELSAVE_RETURN_FMA(fname,params) \
        {\
            ae_int_t cached_cpuid = ae_cpuid();\
            _ALGLIB_KSV_RETURN_FMA(fname,params)\
        }
...
void sparsecreatecrsemptybuf(ae_int_t n,
     sparsematrix* s,
     ae_state *_state);
void sparsecreatecrsfromcrsrangebuf(const sparsematrix* src,
     ae_int_t r0,
     ae_int_t r1,
     sparsematrix* dst,
     ae_state *_state);
...
extern const xparams &no_backend_linalg;
extern const xparams &backend_dss;
...
*************************************************************************/
void sparsecreatecrsfromcrsrangebuf(const sparsematrix &src, const ae_int_t r0, const ae_int_t r1, sparsematrix &dst, const xparams _xparams = alglib::xdefault);
...
// a function that sets global flags that control threading and vendor backends usage
void setglobalflags(const xparams settings);

// an older version of setglobalflags()
...
void spchol_propagatefwd(/* Real    */ const ae_vector* x,
     ...
     ae_int_t simdwidth,
     ae_state *_state);
extern ae_bool (*spchol_updatekernelabc4)(ae_vector*,ae_int_t,ae_int_t,ae_int_t,ae_int_t,ae_int_t,ae_int_t,ae_int_t,const ae_vector*,ae_int_t,const ae_vector*,const ae_vector*,ae_int_t,ae_state*);
...
#define AE_COMPILE_IBETAF
#define AE_COMPILE_STUDENTTDISTR
#define AE_COMPILE_STUDENTTTESTS
#define AE_COMPILE_NEARUNITYUNIT
#define AE_COMPILE_BINOMIALDISTR
#define AE_COMPILE_STEST
#define AE_COMPILE_BASESTAT
#define AE_COMPILE_NTHEORY
#define AE_COMPILE_FTBASE
...
#define AE_COMPILE_MINNLC
#define AE_COMPILE_LPQPPRESOLVE
#define AE_COMPILE_BBGD
#endif

#ifdef AE_COMPILE_MINLPSOLVERS
#define AE_PARTIAL_BUILD
#define AE_COMPILE_APSERV
#define AE_COMPILE_SCODES
#define AE_COMPILE_ABLASPBL
